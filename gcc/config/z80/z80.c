...
{
  return gen_rtx_REG (TYPE_MODE (valtype), Z80_H);
}
		    ...
		    bool named ATTRIBUTE_UNUSED)
{
  ...
  printf("function_arg: %d %d\n", *cum, mode);
  if (*cum < Z80_F && *cum + MOXIE_FUNCTION_ARG_SIZE(mode,type) >= Z80_L)
    if(MOXIE_FUNCTION_ARG_SIZE(mode,type) > 2)
      return NULL_RTX;
    else
      *cum = Z80_H;
  if (*cum <= Z80_L)
			    ...
			    const_tree type, bool named ATTRIBUTE_UNUSED)
{
  ...
  printf("function_arg_advance: %d", *cum);
  *cum = (*cum < Z80_L
//          ? *cum + ((3 + MOXIE_FUNCTION_ARG_SIZE (mode, type)) / 4)
          ? *cum + MOXIE_FUNCTION_ARG_SIZE (mode, type)
	  : *cum);
  if(*cum == Z80_F)
    *cum += 1;
  printf(" -> %d\n", *cum);
